<?php
/*
 * input: $array es un array que voy a dibujar output: $salida es un string con
 * la tabla html
 */
...
class usuariosModel {
	...
	public function procesarUpdate($proyecto, $config, $db) {
		...
		if ($_FILES ['photo'] ['name'] != '') {
			...
			unlink ( $_SERVER ['DOCUMENT_ROOT'] . $proyecto . "/" . $imagen_anterior );
			// Subir imagen nueva
			...
			while ( file_exists ( $destino ) ) {
				$count ++;
				$destino = $_SERVER ['DOCUMENT_ROOT'] . $proyecto . "/" . $partes_ruta ['filename'] . "-" . $count . "." . $partes_ruta ['extension'];
			}
			if ($count == 0) {
	...
	public function procesarDelete($proyecto, $db) {
		// Crear array out
		$out = array ();
		foreach ( $_POST as $key => $value ) {
			if (is_array ( $value ))
